...
def test_setcomprehension(sequences):
    assert set_comprehension(sequences) == {
        c
        for sequence in sequences for c in sequence
    }

def test_double2(sequences):
    assert two_operations(sequences) == [
        len(s.replace("a", "")) for s in sequences
    ]
...
def test_filter(sequences):
    assert len_filter(sequences) == [
        s.count("a") for s in sequences if len(s) > 5
    ]


def test_zip(sequences, ages):
    assert age_filter(sequences, ages) == [
        s.count("a") for s, a in zip(sequences, ages) if a >= 15
    ]
